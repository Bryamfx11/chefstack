...
// si llega un token Firebase valido, autentica al usuario (su uid)
public class FirebaseAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (header != null && header.startsWith(PREFIJO)) {
            ...
            try {
                String uid = verifier.verificarYObtenerUid(token);
                var auth = new UsernamePasswordAuthenticationToken(
                        uid, null, AuthorityUtils.createAuthorityList("ROLE_USER"));
                SecurityContextHolder.getContext().setAuthentication(auth);
            } catch (Exception ex) {
                // token invalido: sigue como anonimo
                log.debug("Token Firebase rechazado: {}", ex.getMessage());
            }
        }
